...
class Hotel_Booking_Stripe_Gateway extends Hotel_Booking_Payment_Gateway {
	...
	public function ajax_confirm_payment() {
		...
		try {
			...
			$notification->send_payment_email( $booking_id, 'completed', $payment_id, 'Stripe' );
			$notification->send_admin_new_booking_notification( $booking_id );
			$notification->send_admin_payment_notification( $booking_id, $payment_id, 'Stripe' );
	...
	private function handle_payment_succeeded( $payment_intent ) {
		...
		$notification = Hotel_Booking_Notification_Manager::get_instance();
		$notification->send_confirmation_email( $booking_id );
		$notification->send_payment_email( $booking_id, 'completed', $payment_intent->id, 'Stripe' );
		$notification->send_admin_new_booking_notification( $booking_id );
		$notification->send_admin_payment_notification( $booking_id, $payment_intent->id, 'Stripe' );
	}
